...
export default function SellerDashboardPage() {
  ...
  const [seller, setSeller] = useState<PhoneNumberInfo | null>(null);
  ...
  useEffect(() => {
    const fetchSeller = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const data: PhoneNumberInfo[] = await response.json();
          const match = data.find((item) => item.id === phoneNumberId || item.realPhoneNumberId === phoneNumberId);
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="rounded-2xl border border-dashed border-purple-200 p-6 bg-purple-50/50 min-w-[260px]">
              ...
              <div className="mt-4 text-sm text-gray-500">
                ID de conexión:
                <span className="ml-2 font-mono text-gray-800">{phoneNumberId}</span>
              </div>
            </div>
          </div>
        </section>
